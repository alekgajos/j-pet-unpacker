...
//     bool kCapTimeAtThreshold = true; // upacker filter, rejects samples that calculated time are larger than threshold
//     uint32_t kTimeCapThreshold = 50000000; // ps
...
//             // parse 4 words from the concentrator's headers
//             for (uint32_t ci=0; ci < 4 && qi < raw_data.size(); ci++, qi++ ) {
//                 // do nothing, skip the concentrators headers
//                 // TODO fill the meta_t. maybe do some comparision

//                 if ( ci == 2 ) {
//                     std::cout << reverse_DJ(raw_data[qi]) << std::endl;
//                     uint32_t buff = reverse_DJ(raw_data[qi]);
//                     if ( (buff & 0xf0ffffff) != 0xA0030000 ) {
//                         std::cout << "Expected CONCID, got nothing..." << std::endl;
//                         // getchar();
//                         break;
//                     }
...
//             // loop over endpoints...
//             for (; qi < raw_data.size(); ) {
//                 // get endpoint data size
//                 uint32_t buff = reverse_TDC(raw_data[qi]);
//                 uint32_t endp_id = buff >> 16;
//                 uint32_t endp_len = buff & 0xffff;

//                 std::cout << std::hex << endp_id << " " << endp_len << std::endl;

//                 // before moving on check if this is endpoint
//                 if ( (endp_id & 0xF00F) == 0xA000 || (endp_id & 0xF00F) == 0xB000 || (endp_id & 0xF00F) == 0xC000) {
//                     // do nothing, this is endpoint
//                     std::cout << "This is endpoint..." << std::endl;
//                 } else if ( endp_id == 0xffff && endp_len != 0xffff ) {
//                     qi += endp_len; // skip the invalid name endpoint
//                     std::cout << "CDC packet name..." << std::endl;
//                     error_cntr++;
//                 } else {
//                     // if here, it means the current data sample does not come from endpoint.
//                     // it might be concentrator, padding or next queue.
//                     std::cout << "Not endpoint or err, breaking from queue loop" << std::endl;
//                     break; 
//                 }
...
//                             std::cout << std::dec << qi << " " << std::hex << reverse_TDC(raw_data[qi]) << " " << ei << " " << endp_len << std::endl;
...
//                         for (uint32_t ei=0; qi < raw_data.size() && ei < endp_len; (qi % 2 == 1 ? ei++ : ei=ei), qi++ ) {
//                             // TODO decoding                                           ^ (!) increment only on even word
//                             uint32_t low_word;
//                             uint32_t high_word;
...
//         while (i != std::end(v)) {  
//             // erase in case of coding error
//             if ( i->channel_id > 104 ) { // || get_fine(i->sample) == 127 
